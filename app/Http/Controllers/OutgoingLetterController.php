...
class OutgoingLetterController extends Controller
{
    ...
    public function store(StoreLetterRequest $request): RedirectResponse
    {
        try {
            ...
            $latestSequenceQuery = Letter::withTrashed()
                ->where('year', date('Y'))
                ->where('type', LetterType::OUTGOING->type());

            if (in_array(DB::connection()->getDriverName(), ['mysql', 'mariadb'], true)) {
                $latestSequence = (int) $latestSequenceQuery
                    ->selectRaw("MAX(CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(reference_number, '/', 2), '/', -1) AS UNSIGNED)) as max_sequence")
                    ->value('max_sequence');
            } else {
                $latestSequence = $latestSequenceQuery
